...
#             st.markdown(f"**#{r['rank']}** · similarity: `{r['score']:.2f}`")
#             if r.get("name"):
#                 st.caption(r["name"])


# def main():
#     st.title("🧵 Saree Visual Search")
#     st.caption(
#         "Chat naturally, upload a saree photo or paste an image URL, and I'll find "
#         "visually similar sarees from the catalog."
#     )
...
#         st.session_state.chat_history = []  # list of LangChain BaseMessage
#     if "display_log" not in st.session_state:
#         st.session_state.display_log = []  # list of dicts for rendering

#     # -- Sidebar: image input -------------------------------------------------
#     st.sidebar.header("Query image")
#     uploaded_file = st.sidebar.file_uploader("Upload a saree image", type=["jpg", "jpeg", "png", "webp"])
...
#         st.sidebar.image(query_image_preview, caption="Query image", use_container_width=True)
...
#         st.session_state.display_log.append(
#             {"role": "assistant", "text": reply, "results": results}
#         )
